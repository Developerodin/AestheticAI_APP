...
const AppointmentsDetails = () => {
  ...
  const endMeeting = async () => {
    ...
    try {
      ...
      console.log('File name ===>', File, sid, isHost, dataid);
  ...
  return videoCall && channelName && token ? (
    ...
    <ScrollView style={styles.container}>
      <View style={styles2.headerContainer}>
        ...
        <TouchableOpacity style={styles2.calendarButton}></TouchableOpacity>
      </View>
      ...
      <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
        ...
        <View
          ...
          <Text style={{fontSize: 14, fontWeight: 700}}>
            {isHost ? 'Start Call As Host' : 'Start Call As Client'}
          </Text>

          {!isHost && (
            <View>
              <Text style={{marginTop: 30}}>Enter Video Id</Text>
              <TextInput
                value={dataid} // Bind the value of the input to the state
                onChangeText={text => setDataId(text)}
                style={{
                  borderWidth: 1,
                  borderColor: 'grey',
                  height: 30,
          ...
          <Text>{loading ? 'processing video...' : msg}</Text>
        </View>
      </View>
    </ScrollView>
  );
